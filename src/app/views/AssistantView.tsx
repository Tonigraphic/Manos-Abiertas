            ...
            <motion.div key="trans" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="max-w-4xl mx-auto space-y-6">
              <Card>
                <CardBody className="space-y-4">
                  ...
                  />
                  <Button onClick={handleTranslate} className="w-full py-6 text-lg font-bold" disabled={!translatorInput}>
              ...
              {translatedText && (
                <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
                  <Card className="bg-[var(--color-primary-50)] border-[var(--color-primary-200)] border-2">
                    <CardBody>
                      <p className="text-[10px] font-bold text-[var(--color-primary-600)] uppercase mb-2">Glosa para Persona Sorda:</p>
                      <p className="text-3xl font-black text-[var(--color-primary-900)] leading-tight">{translatedText}</p>
                    </CardBody>
                  </Card>
                </motion.div>
              )}
            ...
            <motion.div key="rec" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-4">
                <div className="relative aspect-video bg-neutral-900 rounded-3xl overflow-hidden shadow-2xl border-4 border-white group">
                  ...
                  {/* VIDEO: Siempre visible cuando está activo */}
                  <video 
                    ref={videoRef} 
                ...
                <Card className="h-full border-2 border-neutral-100 shadow-sm">
                  <CardBody className="flex flex-col h-full">
                    <h3 className="font-bold mb-6 flex items-center gap-2 text-neutral-700">
                      <Sparkles size={18} className="text-amber-500" /> Resultados IA
                    </h3>
                    
                    <div className="text-center py-10 bg-neutral-50 rounded-3xl border-2 border-dashed border-neutral-200 mb-8 min-h-[220px] flex flex-col items-center justify-center">
                      {recState.currentSign ? (
                        <motion.div initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }}>
                          <span className="text-[10px] text-neutral-400 block uppercase font-black tracking-widest mb-1">Seña Detectada:</span>
                          <span className="text-6xl font-black text-[var(--color-primary-600)] block mb-3 uppercase tracking-tighter">
                            {recState.currentSign.sign}
                          </span>
                          <Badge variant="primary">Seguridad: {Math.round(recState.currentSign.confidence * 100)}%</Badge>
                        </motion.div>
                      ) : (
                        <div className="opacity-30 flex flex-col items-center">
                           <Hand size={48} className="mb-3 text-neutral-400" />
                    ...
                    <div className="flex-1 overflow-hidden">
                      <p className="text-[10px] font-black text-neutral-400 uppercase mb-4 tracking-widest border-b pb-2">Historial</p>
                      <div className="space-y-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                        {recState.recognizedSigns.length > 0 ? (
                          ...
                          <p className="text-center text-xs text-neutral-300 py-8 italic">Esperando datos...</p>
                        )}
                      </div>
                    </div>
                  </CardBody>
